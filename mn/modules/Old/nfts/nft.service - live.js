...
let self = module.exports = {
					...
					if(result.length == 0){
						...
						for(let i = 0; i < result.length; i++){
							...
							if(result[i].palletsCount < result[i].data.size){
								...
								ipfsdata.findOneAndUpdate({"ipfsHash" : data.ipfsHash},{"$set": {"nftSaveUserId":mongoose.Types.ObjectId(data.userId), "userType": data.userType}},{new: true, upsert: true}).exec();
								...
								saveNftsPalletsInChain.findOne({"ipfsHash": data.ipfsHash}, async (err, verifyData) => {
									...
									// if(verifyData == null){
										...
										ipfsdata.findOne({"ipfsHash": data.ipfsHash}, async (err, newData) => {
	...
	hashBurnByUserId: async(data)=> {
		...
		ipfsverifydata.findOneAndUpdate({"ipfsHash":data.ipfsHash},{ "$set": { "status": 3, "nftBurnTransactionHash": data.nftBurnTransactionHash, "nftBurnByUserId": data.nftBurnByUserId, "userType": data.userType, "nftBurnByUserIdDate": new Date()}},{new:true, upsert:true})
		.exec(async (err, result) => {			
			if(err){
				let obj = {"status":0,"data":err}
                deferred.resolve(obj)
	...
	verifyHashStatus: async(data)=> {
		...
		if(data.boxType == "containers"){
			...
			assignContainerToOthersData.find(
				{
					...
					if(err){
						...
						if(result.length > 0){
							...
							// Start old One
							let updateData = { $set: {"status": 2, "nftVerifyUserFlag": 1, "nftVerifyByUserId": data.nftVerifyByUserId, "nftBurnByUserId": "", "userType": data.userType, "nftVerifyByUserIdDate": new Date() } }
					
							ipfsverifydata.findOneAndUpdate({"ipfsHash":ipfsHash},updateData,{new:true, upsert:true}).exec(async (err, newResult) => {			
								if(err){
									let obj = {"status":0,"data":err}
									deferred.resolve(obj)
								} else {
							...
							// Start old One
							let updateData = { $set: {"status": 1, "nftVerifyUserFlag": 2, "nftBurnByUserId": "", "userType": data.userType, "nftVerifyByUserIdDate": new Date() } }
			...
			ipfsverifydata.find({"nftSaveUserId": data.nftVerifyByUserId[0].verifyByUserId}, async (err, result) => {
				if(err){
					let obj = {"status":0,"data":err}
					deferred.resolve(obj)
				} else {
					...
					// } else {
						...
						let updateData = { $set: {"status": 2, "nftVerifyUserFlag": 2, "nftVerifyByUserId": data.nftVerifyByUserId, "nftBurnByUserId": "", "userType": data.userType, "nftVerifyByUserIdDate": new Date() } }
