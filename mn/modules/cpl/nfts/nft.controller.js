...
const hashBurnByUserIdParams = ['ipfsHash','nftBurnByUserEmailId'];
...
module.exports = {
	updateDeliveryIntentNoData: (req, res,next) => {
        try {

			nftService.updateDeliveryIntentNoData(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
	...
	getInvoicePrintData: (req, res,next) => {
        try {

			nftService.getInvoicePrintData(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	listPrintedAppTransactionData: (req, res,next) => {
        try {

			nftService.listPrintedAppTransactionData(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	savePrintedAppTransactionData: (req, res,next) => {
        try {

			nftService.savePrintedAppTransactionData(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
						...
						data.fetchData = result.data					
						var obj = { responseStatus: 0, responseMsgCode: "Already Exists ! Delivery Intent No.", "responseData":{}}
					} else {
						data.error = result.data
						var obj = { responseStatus: 0, responseMsgCode: "failure", "responseData":data}
					}					
	...
	getPrintedNfts: (req, res,next) => {
        try {

			nftService.getPrintedNfts(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	updateStatusGenratedNfts: (req, res,next) => {
        try {

			nftService.updateStatusGenratedNfts(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	genrateAllDocsPdf: (req, res,next) => {
        try {

			nftService.genrateAllDocsPdf(req.body).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	genratePdfs: (req, res,next) => {
        try {
			...
			nftService.genratePdfs(req.body.fdalebelId,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	sendDataToPrinter: async (req, res) => {
        try {
			
			nftService.sendDataToPrinter(req.body,"DB_"+req.headers['typedata'], req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
	...
	export: async (req, res) => {
        try {
			...
			nftService.export(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	changeReportStatus: async (req, res) => {
        try {
			...
			nftService.changeReportStatus(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getNftsVerifyUserReport: async (req, res) => {
        try {
			...
			nftService.getNftsVerifyUserReport(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getLastOrderByUserId: async (req, res) => {
        try {
			...
			nftService.getLastOrderByUserId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getContainerDataAsPerUserType: async (req, res) => {
        try {
			...
			nftService.getContainerDataAsPerUserType(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
	...
	getIpfsDataAsPerType: async (req, res) => {
        try {
			...
			nftService.getIpfsDataAsPerType(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getCompanyIdIpfs: async (req, res) => {
		...
		try {
			...
			nftService.getCompanyIdIpfs(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getOrderByProductId: async (req, res) => {
        try {
			...
			nftService.getOrderByProductId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	getNftsReportAsPerUserId: async (req, res) => {
        try {
			...
			nftService.getNftsReportAsPerUserId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	saveNftsData: async (req, res) => {
        try {
			...
			nftService.saveNftsDataNew(req.body,"DB_"+req.headers['typedata']).then(result => {
	...
	verifyHashUserNftsData: async (req, res) => {
        try {
			...
			nftService.verifyHashUserNftsData(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.getData = result.data
	...
	viewAssignDataByWarehouseAndDistributerId: async (req, res) => {
		try {
			...
			// nftService.viewAssignDataByWarehouseAndDistributerId(req.body,"DB_"+req.body.userRelationSlug).then(result => {	
			nftService.viewAssignDataByWarehouseAndDistributerId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	viewAssignDataByManufacturerId: async (req, res) => {
		try {
			...
			nftService.viewAssignDataByManufacturerId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	hashBurnByUserId: async (req, res) => {
        try {
			...
			nftService.hashBurnByUserId(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
	...
	assignNftsByUserId: async (req, res) => {
        try {
			...
			nftService.assignNftsByUserId(req.body,"DB_"+req.headers['typedata']).then(result => {
	...
	verifyHash: async (req, res) => {
        try {
			...
			nftService.verifyHashStatus(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.insertData = result.data
	...
	assignContainerToOthers: async (req, res) => {
        try {
			...
			nftService.assignContainerToOthers(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.insertData = result.data					
    ...
    assignContainer_old: async (req, res) => {
        try {
			...
			const validation = await Promise.all([nftService.checkAssignItem(req.body,"DB_"+req.headers['typedata'])]);
			...
			nftService.assignContainer(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.insertData = result.data					
	...
	assignContainer: async (req, res) => {
        try {
			...
			nftService.assignContainer(req.body,"DB_"+req.headers['typedata'],req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.insertData = result.data					
	...
	viewContainerByFactory: (req, res) => {
        try {
			...
			nftService.viewContainerByFactory(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	viewContainerFactoryById: (req, res) => {
        try {
			...
			nftService.viewContainerFactoryById(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	viewContainerCompanyFactoryById: (req, res) => {
        try {
			...
			nftService.viewContainerCompanyFactoryById(req.body,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	viewContainerByfdalebelId: (req, res,next) => {
        try {
			...
			nftService.viewContainerByfdalebelId(req.body.fdalebelId,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data.fetchData = result.data					
	...
	getdatafromipfshash: (req, res) => {
        try {
			...
			nftService.getdatafromipfshash(req.body.ipfshash,"DB_"+req.headers['typedata']).then(result => {				
				if (result.code != 11000) {
					var data = {}
					if (result.status == 1) {
						data = result.data					
