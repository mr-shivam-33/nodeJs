...
let self = (module.exports = {
  ...
  assignCustomerByUser: (data) => {
    var deferred = Q.defer();
    let [PermissionInfo, CustomerVerifyInfo] = data.schema;
    delete data.schema;
    UserMemberInfo.findOne(
      { email: data.email },
      { _id: 1, email: 1 },
      (err, result) => {
        if (err || result == null) {
          ...
          CustomerInfo.findOneAndUpdate(
            ...
            { $push: { assignToCustomer: result._id } },
            { new: true },
            (err, result) => {
              if (err) {
                let obj = { status: 0, data: err };
                ...
                CustomerVerifyInfo.findOneAndUpdate(
                  ...
                  },
                  {
                    $set: {
                      ...
                      "nftVerifyByUserId.$.assignToUserType": Number(
                        data.userType
                      ),
                    },
                  },
          ...
          UserInfo.aggregate([
            {
              $match: {
                _id: {
                  $in: ["617bccecd2856625a54add5c", "618a50ce3242930fc3c37065"],
                },
              },
            },
          ]).exec((err, resultData) => {
        ...
        $match: {
          $or:
            userType == 1
              ? [
                  {
                    companyUserId: mongoose.Types.ObjectId(userId),
                    status: 1,
                    userStatusDelete: 0,
                  },
                  {
              ...
              : [
                  {
                    companyUserId: mongoose.Types.ObjectId(userId),
                    status: 1,
                    userType: Number(userType.toString()),
                    userStatusDelete: 0,
                  },
                  {
                    companyUserId: mongoose.Types.ObjectId(userId),
  ...
  userModifyPermission: (info) => {
    ...
    UserInfo.findOneAndUpdate(
      ...
      { $set: { permissions: info.permissions } },
      { new: true },
      (err, result) => {
        if (err) {
          let obj = { status: 0, data: err };
  ...
  findUserEmailId: (email) => {
    ...
    UserInfo.findOne(
      { email: email },
      { userName: 1, email: 1, status: 1, userStatusDelete: 1 },
      (err, result) => {
        if (err) {
          deferred.resolve(err);
        } else {
  ...
  deleteuserByCompany: (userId) => {
    ...
    UserInfo.findOneAndUpdate(
      ...
      { $set: { status: 0, userStatusDelete: 1 } },
      { new: true },
      (err, result) => {
        if (err) {
          let obj = { status: 0, data: err };
  ...
  userAddAdress: (userId, userAddressInfo) => {
    ...
    AddressInfo.findOneAndUpdate(
      { _id: mongoose.Types.ObjectId(userId) },
      { $push: { useraddressInfo: userAddressInfo } },
      { new: true },
      (err, result) => {
        if (err) {
          let obj = { status: 0, data: err };
  ...
  userEditAddress: (addressManageId, userId, addressData) => {
    ...
    AddressInfo.findOneAndUpdate(
      ...
      {
        $set: {
          "useraddressInfo.$.address": addressData.address,
          "useraddressInfo.$.location": addressData.location,
          "useraddressInfo.$.state": addressData.state,
          "useraddressInfo.$.pin": addressData.pin,
          "useraddressInfo.$.mobile": addressData.mobile,
          "useraddressInfo.$.status": addressData.status,
        },
      },
      { new: true },
      (err, result) => {
  ...
  addUserMember: (data) => {
    var deferred = Q.defer();
    data.relationUserId =
      data.relationUserId != undefined ? data.relationUserId : "";
    data.companyUserId =
      data.companyUserId != undefined ? data.companyUserId : "";
    ...
    UserMemberInfo.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          familyRelation: memberData.familyRelation,
        },
      },
      (err, result) => {
        if (err || result == null) {
  ...
  contractManufacturer: async (userId, data) => {
    var deferred = Q.defer();
    var relationUserId = [];
    let [customUserInfoNew1] = data.customUserInfoNew1;
    // console.log(customUserInfoNew1.collection.collectionName);
    // return
            ...
            if (newResult) {
              ManufacturerInfo.findOneAndUpdate(
                ...
                },
                {
                  $push: {
                    factoryInfo: {
                      factoryName: data.firstName,
                      manufacturerBYId: newResult._id,
                      companyId: userId,
                    },
                  },
                },
                ...
                async (err, result1) => {
                  if (err || result1 == null) {
                    ...
                    await customUserInfoNew1.create({
                      firstName: newResult.firstName,
                      lastName: newResult.lastName,
                      email: newResult.email,
                      hash: newResult.hash,
                      sessionToken: newResult.sessionToken,
                      uniqueId: newResult.uniqueId,
                      status: newResult.status,
                      memberRegisterStatus: newResult.memberRegisterStatus,
                      userStatusDelete: newResult.userStatusDelete,
                      loginAs: newResult.loginAs,
                      countryCode: newResult.countryCode,
                      userSlug: newResult.userSlug,
                      relationUserId: newResult.relationUserId,
                      assignToCustomer: newResult.assignToCustomer,
                      _id: newResult._id,
                      createdAt: newResult.createdAt,
                      updatedAt: newResult.updatedAt,
                    });
                    // await customUserInfoNew1.findOneAndUpdate({"email": newResult.email}, newResult, {"new":true, upsert:true}).exec();
      ...
      (err, result) => {
        if (err) {
          ...
          if (result == null || result == undefined) {
            ...
            let obj = { status: 1, data: result };
            deferred.resolve(obj);
            result.profileStepsCompleted =
              result.profileStepsCompleted == null
                ? []
                : result.profileStepsCompleted;
            result.profilePoint =
              result.profilePoint == null ? 0 : result.profilePoint;
            result.profilePointUpdateCount =
              result.profilePointUpdateCount == null
                ? 0
                : result.profilePointUpdateCount;
